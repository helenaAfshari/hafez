...
class DetailsGhazaliatHafezBloc
    extends Bloc<DetailsGhazaliatHafezEvent, DetailsGhazaliatHafezState> {
  DetailsGhazaliatHafezBloc() : super(DetailsGhazaliatHafezInitialState()) {
    List<DetailsGhazaliatHafezModel> detailsGhazaliatHafez = [];
    on<DetailsGhazaliatHafezEvent>(
      (event, emit) async {
        emit(DetailsGhazaliatHafezLoadingState());
        if (event is LoadedddEvent) {
          try {
            final detailsGhazaliatResponse =
                await serviceLocator<DetailsGhazaliatHafezRepository>()
                    .detailsGhazaliatHafezRepository(event.selectedPoemIdd);
            print("DetailsGhazaliatRespons${detailsGhazaliatResponse}");
            detailsGhazaliatHafez =
                (detailsGhazaliatResponse.data['data'] as List)
                    .map((e) => DetailsGhazaliatHafezModel.fromJason(e))
                    .toList();
            if (detailsGhazaliatResponse.statusCode == 200) {
              print("DetailsGhazaliat");
              emit(DetailsGhazaliatHafezSuccesState(
                detailsGhazaliatHafez,
              ));
              print("DetailsGhazaliatHafez${detailsGhazaliatHafez}");
            } else {
              emit(DetailsGhazaliatHafezErrorState("Not Valid"));
            }
          } catch (e) {
